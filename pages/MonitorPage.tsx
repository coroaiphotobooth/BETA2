...
const MonitorPage: React.FC<MonitorPageProps> = ({ onBack, activeEventId, eventName, monitorSize = 'medium', theme = 'physics' }) => {
    ...
    const loadData = async () => {
      try {
        ...
        if (theme === 'slider' && photos.length > 0) {
            if (latestIdRef.current !== photos[0].id) {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-[#050505] overflow-hidden overscroll-none touch-none">
      ...
      {/* 4. SLIDER MODE (Split View: Hero Top + Slider Bottom) */}
      {theme === 'slider' && sliderActiveItem && (
         <div className="absolute inset-0 z-10 flex flex-col">
            
            {/* TOP: HERO SECTION */}
            <div className="flex-1 relative flex items-center justify-center overflow-hidden">
               {/* Ambient Background based on active image */}
               <div key={`bg-${sliderActiveItem.id}`} className="absolute inset-0 z-0 transition-opacity duration-1000">
                  <img src={sliderActiveItem.imageUrl} className="w-full h-full object-cover opacity-20 blur-2xl transform scale-110" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent" />
               </div>

               {/* Main Content Area */}
               <div className="relative z-10 w-full h-full flex flex-col md:flex-row items-center justify-center p-8 gap-8 md:gap-16">
                  
                  {/* Hero Image Frame */}
                  <div key={`hero-${sliderActiveItem.id}`} className="relative h-full max-h-[70vh] aspect-[9/16] animate-[fadeInScale_0.6s_ease-out]">
                      <div className="absolute inset-0 bg-purple-600/20 blur-xl rounded-2xl animate-pulse" />
                      <div className="relative h-full w-full rounded-2xl overflow-hidden border-2 border-white/20 shadow-[0_0_50px_rgba(147,51,234,0.3)] bg-black">
                         <img 
                  ...
                  {/* Info & QR Panel */}
                  <div key={`info-${sliderActiveItem.id}`} className="flex flex-col items-center gap-6 animate-[slideInRight_0.6s_ease-out_0.1s]">
                      <div className="text-center">
                         <h2 className="text-4xl md:text-5xl font-heading text-white neon-text italic uppercase tracking-tighter mb-2">
                           {sliderActiveItem.conceptName}
                         </h2>
                         <p className="font-mono text-purple-300 text-xs tracking-[0.5em] uppercase">
                           {new Date(sliderActiveItem.createdAt).toLocaleTimeString()}
                         </p>
                      </div>

                      <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/20 shadow-2xl group transition-transform hover:scale-105">
                          <div className="bg-white p-3 rounded-xl shadow-inner">
                            <img 
                              src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(getShareUrl(sliderActiveItem))}`} 
                              className="w-48 h-48 md:w-56 md:h-56 object-contain"
                            />
                          </div>
                      </div>
                      
                      <div className="flex items-center gap-3 text-white/80 border border-white/10 px-6 py-3 rounded-full bg-black/40 backdrop-blur">
                        <svg className="w-5 h-5 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4-4m0 0l-4 4m4-4v12" /></svg>
                        <span className="font-heading text-xs tracking-[0.2em] uppercase">SCAN TO DOWNLOAD</span>
                      </div>
                  </div>
               </div>
            </div>
            ...
            {/* BOTTOM: SLIDER STRIP */}
            <div className="h-48 shrink-0 z-20 bg-gradient-to-t from-black via-black/95 to-transparent flex flex-col justify-end pb-6">
               <div className="flex items-center px-8 gap-4 overflow-x-auto scrollbar-hide w-full pb-4 pt-4 mask-fade-sides">
                  {photoItems.slice(0, 30).map((item) => {
                     const isActive = sliderActiveItem.id === item.id;
                     return (
                        <div 
                           key={item.id}
                           onClick={() => setSliderActiveItem(item)}
                           className={`relative flex-shrink-0 w-24 h-36 md:w-32 md:h-48 rounded-lg overflow-hidden cursor-pointer transition-all duration-300 transform ${isActive ? 'scale-110 border-2 border-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.6)] -translate-y-2' : 'border border-white/10 opacity-60 hover:opacity-100 hover:scale-105'}`}
                        >
                           <img 
      ...
      {/* Lightbox Overlay (Only for Physics, Grid, Slider uses custom Hero) */}
